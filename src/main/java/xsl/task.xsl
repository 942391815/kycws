    ...
    <xsl:element name="script">
      <xsl:attribute name="language">Javascript</xsl:attribute>
       if(!adf.Task._connections) {
	     ...
	     adf.Task._ValidationRules=[];
	     adfdojo.connect(window,"unload", function(){
	       adfdojo.forEach(adf.Task._connections, adfdojo.disconnect);
	     });
       }
    </xsl:element>
    ...
    <xsl:choose>
      <xsl:when test="count(/task/task-layouts/task-layout) > 0">
        <xsl:for-each select="/task/task-layouts/task-layout">
          ...
          <xsl:choose>
          <xsl:when test="//show-in-window = 'true' or (//show-in-window = 'false' and position() > 1)">
                      <xsl:element name="script">
              <xsl:attribute name="type">text/javascript</xsl:attribute>
              <xsl:attribute name="language">Javascript</xsl:attribute>
              adfdojo.addOnLoad(function(){adfdojo.parser.parse();}); // To render declarative dijits
            </xsl:element>   
          ...
          <xsl:element name="tr">
            <xsl:element name="td">
              ...
              <xsl:element name="a">
                ...
                <xsl:value-of select="//LocalizedStrings/Task/Help" />                
              </xsl:element>
            </xsl:element>
          </xsl:element>
        </xsl:if>
    ...
    <xsl:element name="script">
      ...
      <xsl:choose>
        <xsl:when test="//task/task-descriptor/@visible = 'true'">
          esriInitItems.push('adf.Task.initTaskWindow("<xsl:value-of select="$id"/>","<xsl:value-of select="$pe"/>","<xsl:value-of select="$winTitle"/>","<xsl:value-of select="$win"/>", <xsl:value-of select="$layout != 'absoluteLayout'"/>,"<xsl:value-of select="//map-id"/>",true)');
        </xsl:when>
        <xsl:otherwise>
          esriInitItems.push('adf.Task.initTaskWindow("<xsl:value-of select="$id"/>","<xsl:value-of select="$pe"/>","","<xsl:value-of select="$win"/>", <xsl:value-of select="$layout != 'absoluteLayout'"/>,"<xsl:value-of select="//map-id"/>",true)');
        </xsl:otherwise>
      </xsl:choose>
    </xsl:element>
  </xsl:template>
