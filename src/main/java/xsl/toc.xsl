    ...
    <xsl:variable name="nodeCnt" select="count(/toc/node)"/>
      ...
      <xsl:choose>
        <xsl:when test="/toc/show-open-result-link = 'true'">
          <xsl:variable name="openLabel"><xsl:value-of select="//LocalizedStrings/Toc/OpenResultLinkText" /></xsl:variable>
                ...
                <xsl:element name="td">
                  <xsl:element name="script">
                    ...
                    function esriOpenResultHandler<xsl:value-of select="$tocId" />(filename, id) {
                      if (! filename || ! id)
                        alert("<xsl:value-of select="//LocalizedStrings/Toc/FileUploadFailed" />");
                      else {
                        ...
                        var xmlHttp = adf.Utils.sendAjaxRequest(url, params, true, function() { adf.Controls.processPostBack(xmlHttp); });
                      }
                    }
                  </xsl:element>
                  <xsl:element name="a">
                    <xsl:attribute name="href">javascript:void(0);</xsl:attribute>
                    <xsl:attribute name="onclick">adf.UploadUtil.showUploadWindow('<xsl:value-of select="$openLabel" />', esriOpenResultHandler<xsl:value-of select="$tocId" />);</xsl:attribute>
      ...
      <xsl:element name="script">
        ...
        function esriTocInit<xsl:value-of select="$tocId"/>() {
          var <xsl:value-of select="$tocId"/> = new adf.Toc("<xsl:value-of select="$tocId"/>", document.getElementById("<xsl:value-of select="$tocCellId"/>"), "<xsl:value-of select="//map-id" />");
        ...
        <xsl:if test="/toc/toc-model-type = 'WebResultsToc' and count(/toc/toc-model-type/timer) > 0"> 
