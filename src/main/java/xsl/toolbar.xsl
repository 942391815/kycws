    ...
    <xsl:element name="script">
      ...
      <xsl:choose>
        <xsl:when test="/toolbar/toolbar-style = 'IMAGEONLY'">
          var <xsl:value-of select="$toolbarId"/> = new adf.ImageToolbar("<xsl:value-of select="$toolbarId"/>", null, "<xsl:value-of select="/toolbar/map-id"/>");
        </xsl:when>
        <xsl:when test="/toolbar/toolbar-style = 'IMAGEANDTEXT'">
          var <xsl:value-of select="$toolbarId"/> = new adf.ImageAndTextToolbar("<xsl:value-of select="$toolbarId"/>", null, "<xsl:value-of select="/toolbar/map-id"/>");
        </xsl:when>
        <xsl:otherwise>
          var <xsl:value-of select="$toolbarId"/> = new adf.TextToolbar("<xsl:value-of select="$toolbarId"/>", null, "<xsl:value-of select="/toolbar/map-id"/>");
        </xsl:otherwise>
      </xsl:choose>
      <xsl:choose>
          ...
          <xsl:when test="@type =  'COMMAND'">
            <xsl:choose>
              <xsl:when test="@onclick != ''">
                var <xsl:value-of select="$toolid"/> = new adf.MapServerAction("<xsl:value-of select="$toolid" />", "<xsl:value-of select="@tool-text" />", <xsl:value-of select="@onclick" />);
              </xsl:when>
              <xsl:otherwise>
                var <xsl:value-of select="$toolid"/> = new adf.MapServerAction("<xsl:value-of select="$toolid" />", "<xsl:value-of select="@tool-text" />");
              </xsl:otherwise>
            </xsl:choose>
            <xsl:if test="@style != ''">
              <xsl:value-of select="$toolid"/>.style = "<xsl:value-of select="@style"/>";
      ...
      <xsl:if test="/toolbar/active-tool != ''">
        adf.Controls.maps['<xsl:value-of select="/toolbar/map-id"/>'].setCurrentToolItem(<xsl:value-of select="/toolbar/active-tool"/>, "<xsl:value-of select="$toolbarId"/>");
      </xsl:if>
      }
