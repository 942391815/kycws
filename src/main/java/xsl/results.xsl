...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- XSL file for Result Control -->
  <xsl:import href="core.xsl"/>
  <xsl:output method="html" version="1.0" encoding="UTF-8" indent="yes" />

  <!--
    To customize the appearance of the Result, enter values for each attribute you wish to customize in their
  ...
  <!-- Read in the value for first occurrence (of result control on page) from the input XML file -->
  ...
  <!-- Begin html output -->
  <xsl:template match="/">
    <xsl:comment>Result Control Renderer Start (<xsl:value-of select="$tocId"/>)</xsl:comment>
    ...
    <xsl:variable name="nodeCnt" select="count(/toc/node)"/>
      ...
      <xsl:choose>
        <xsl:when test="/toc/show-open-result-link = 'true'">
          ...
          <xsl:element name="table">
           <xsl:attribute name="id">gpResultsUpload</xsl:attribute>
            <xsl:if test="$style != ''">
                ...
                <xsl:element name="td">
                  <xsl:element name="script">
                    ...
                    function esriOpenResultHandler<xsl:value-of select="$tocId" />(filename, id) {
                      if (! filename || ! id)
                        alert(dojo.i18n.getLocalization('resource','adf').file_upload_fail);
                      else {
                        var map = adf.Controls.maps["<xsl:value-of select="//map-id" />"];
                        var url = adf.Utils.getServerUrl(map.formId);
                        var params = "gpAsyncTaskResults=gpAsyncTaskResults&amp;formId=" + map.formId + "&amp;tocId=<xsl:value-of select="$tocId" />&amp;getResult=getResult&amp;id=" + id+"&amp;"+adf.Utils.buildRequestParams(map.formId);
                        adf.Utils.sendAjaxRequest(url, params, true, adf.Controls.processPostBack);
                        }
                    }
                  </xsl:element>
                  <xsl:element name="a">
                    <xsl:attribute name="href">javascript:void(0);</xsl:attribute>
                    <xsl:attribute name="onclick">adf.UploadUtil.showUploadWindow('<xsl:value-of select="$openLabel" />', adf.Utils.getFunctionByName('esriOpenResultHandler<xsl:value-of select="$tocId" />', window));</xsl:attribute>
      ...
      <xsl:element name="script">
        ...
        function esriTocInit<xsl:value-of select="$tocId"/>() {
          var <xsl:value-of select="$tocId"/> = new adf.Toc("<xsl:value-of select="$tocId"/>", document.getElementById("<xsl:value-of select="$tocCellId"/>"), "<xsl:value-of select="//map-id" />", new wma.ResultsTocRenderer());
          ...
          <xsl:for-each select="toc/node">
            ...
            <xsl:if test="$contextItemCount > 0" >
              ...
              <xsl:for-each select="content/context-menu/context-item">
                contextMenuItems<xsl:value-of select="$tocId" />_<xsl:value-of select="$vkey" />.push(new adf.ContextMenuItem('<xsl:value-of select="@label"/>','<xsl:value-of select="@value"/>','<xsl:value-of select="@description"/>'));
              </xsl:for-each>
            </xsl:if>
        ...
        <xsl:if test="/toc/toc-model-type = 'MapViewerResults' and count(/toc/toc-model-type/timer) > 0"> 
        	<xsl:for-each select="/toc/toc-model-type/timer">
        		<xsl:choose>
        			<xsl:when test="@name = 'GPTimer'">
			          function esriWebResultsAsyncTimer<xsl:value-of select="$tocId"/>() {
			            new adf.GPAsyncTaskResultsTimer("EsriWebResults_<xsl:value-of select="$tocId"/>_Timer", "<xsl:value-of select="$tocId"/>", 5);
			          }
    ...
    <xsl:comment>Result Control Renderer End (<xsl:value-of select="$tocId"/>)</xsl:comment>
  </xsl:template>
</xsl:stylesheet>