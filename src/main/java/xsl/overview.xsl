  ...
  <xsl:template match="/">
    ...
    <xsl:element name="script">
      ...
      function esriOverviewInit<xsl:value-of select="$overviewId"/>() {
        var <xsl:value-of select="$overviewId" /> = new adf.Overview("<xsl:value-of select="$overviewId" />", null, "<xsl:value-of select="//map-id" />", <xsl:value-of select="//width" />, <xsl:value-of select="//height" />);
