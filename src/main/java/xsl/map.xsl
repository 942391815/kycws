...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="mapcellid">adf.MapCell_<xsl:value-of select="$mapId"/></xsl:variable>
    ...
    <xsl:element name="script">
      ...
      function esriMapInit<xsl:value-of select="$mapId"/>() {
        var map = new adf.Map("<xsl:value-of select="$mapId"/>", null, "<xsl:value-of select="/map/form-id" />", <xsl:value-of select="/map/width" />, <xsl:value-of select="/map/height" />);
        ...
        map.extent = new adf.Extent(<xsl:value-of select="/map/extent/@minx"/>, <xsl:value-of select="/map/extent/@miny"/>, <xsl:value-of select="/map/extent/@maxx"/>, <xsl:value-of select="/map/extent/@maxy"/>);
        map.scale = <xsl:value-of select="/map/scale"/>
        map.cacheClientFeatures = <xsl:value-of select="/map/enable-client-feature-caching" />;
        
        map.showCoordinateTracker = <xsl:value-of select="/map/show-coordinate-tracker"/>;
        map.showZoomSlider = <xsl:value-of select="/map/show-zoom-slider"/>;
        map.showNavigator = <xsl:value-of select="/map/show-navigator"/>;
        <xsl:if test="count(/map/north-arrow) = 1">
          map.northArrowUrl = "<xsl:value-of select="/map/north-arrow/@image-url"/>";
        </xsl:if>
        ...
        <xsl:if test="count(/map/tile-levels-count) = 1">
          ...
          map.zoomBeyondCache = <xsl:value-of select="/map/zoom-beyond-cache" />;
        </xsl:if>
        ...
        <xsl:for-each select="/map/data-sources/data-source">
          ...
          <xsl:variable name="mapSourceId" select="@id"/>

          <xsl:choose>
            <xsl:when test="$tile = 1">
              <xsl:value-of select="$ds" /> = new adf.MapSourceTile("<xsl:value-of select="tile/base-url" />", <xsl:value-of select="tile/tile-width" />, <xsl:value-of select="tile/tile-height" />, <xsl:value-of select="tile/levels-count" />, <xsl:value-of select="tile/tile-level" />, <xsl:value-of select="tile/start-tile-column" />, <xsl:value-of select="tile/end-tile-column" />, <xsl:value-of select="tile/tile-column" />, <xsl:value-of select="tile/start-tile-row" />, <xsl:value-of select="tile/end-tile-row" />, <xsl:value-of select="tile/tile-row" />, <xsl:value-of select="tile/offset-x" />, <xsl:value-of select="tile/offset-y" />, "<xsl:value-of select="tile/type" />");
            ...
            <xsl:when test="$dynamic = 1">
              <xsl:value-of select="$ds" /> = new adf.MapSourceDynamic(map); 
              <xsl:if test="count(@transparency) = 1"><xsl:value-of select="$ds" />.imageOpacity = <xsl:value-of select="@transparency" />;</xsl:if>
              <xsl:value-of select="$ds" />._staticImageUrl = "<xsl:value-of select="dynamic/image-url"/>";
              msImages.push("<xsl:value-of select="dynamic/image-url"/>");
            </xsl:when>
          </xsl:choose>

          msNames.push("<xsl:value-of select="$mapSourceId"/>" ); //"<xsl:value-of select="$ds" />");
