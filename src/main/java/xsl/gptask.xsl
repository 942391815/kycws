  ...
  <xsl:template match="/" mode="showInWindow">
    <xsl:if test="//show-in-window = 'true'">
        ...
        <xsl:attribute name="language">Javascript</xsl:attribute>
          <xsl:choose>
            <xsl:when test="//task/task-descriptor/@visible = 'true'">
              esriInitItems.push('adf.Task.initTaskWindow("<xsl:value-of select="$taskCellId"/>","<xsl:value-of select="$pe"/>","<xsl:value-of select="//task/task-descriptor/display-name"/>","<xsl:value-of select="$win"/>",<xsl:value-of select="$layout != 'absoluteLayout'"/>,"<xsl:value-of select="//map-id"/>",true)');
            </xsl:when>
            <xsl:otherwise>
              esriInitItems.push('adf.Task.initTaskWindow("<xsl:value-of select="$taskCellId"/>","<xsl:value-of select="$pe"/>","","<xsl:value-of select="$win"/>",<xsl:value-of select="$layout != 'absoluteLayout'"/>,"<xsl:value-of select="//map-id"/>",true)');
            </xsl:otherwise>
          </xsl:choose>
      </xsl:element>
      ...
      <xsl:for-each select="/task/task-params/task-param[@type = 'GPRecordSet']">
        ...
        <xsl:element name="script">
          <xsl:attribute name="language">Javascript</xsl:attribute>
            esriInitItems.push('adf.Task.initTaskWindow("<xsl:value-of select="$cellFeatures"/>","<xsl:value-of select="$peFeatures"/>","Feature","<xsl:value-of select="$winFeatures"/>", <xsl:value-of select="$layout != 'absoluteLayout'"/>,"<xsl:value-of select="//map-id"/>",null,true)');
        </xsl:element>
      </xsl:for-each>
