...
<head>
	...
	<script type="text/javascript">
		var key, lastValue = "", nodeList = [], isChild = getQueryString("isChild", "${url}");
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(isChild),url:"${ctx}${childUrl}",autoParam:["id="+"${childParam}"]},
		...
		function selectCheckNode(){
			...
			for(var i=0; i<ids.length; i++) {
				var node = tree.getNodeByParam("id", (isChild?"u_":"")+ids[i]);
				if("${checked}" == "true"){
